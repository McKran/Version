...
import {
  ...
  ChevronRight, Grid3x3, List, X, Languages, Globe
} from "lucide-react";
...
const STEPS = [
  { id: 1, label: "Language", icon: Languages },
  { id: 2, label: "Region", icon: MapPin },
  { id: 3, label: "Province", icon: MapPin },
  { id: 4, label: "City", icon: Building2 },
  { id: 5, label: "Crops", icon: Wheat },
];

export default function Onboarding() {
  const { completeOnboarding, setLanguage: setGlobalLanguage } = useSettings();
  ...
  const toggleCrop = (crop: PhCrop) => {
    ...
    setSelectedCrops(prev =>
      prev.some(c => getCropKey(c) === key)
        ? prev.filter(c => getCropKey(c) !== key)
        : [...prev, crop]
    );
  };
        ...
        <div className="bg-card border rounded-3xl shadow-xl overflow-hidden">
          ...
          {step === 1 && (
            <div className="p-6 sm:p-8">
              <h1 className="text-2xl font-bold mb-1">Select Preferred Language / Pumili ng Wika</h1>
              <p className="text-muted-foreground mb-6 text-sm">
                Choose your preferred language for the interface controls and Grownox AI responses.
              </p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 my-4">
                ...
                >
                  ...
                  <p className="text-xs text-muted-foreground leading-relaxed">
                    Full English interface, controls, and Grownox AI agricultural responses.
                  </p>
                </button>
              ...
              <div className="mt-6 p-4 rounded-xl bg-muted/50 border border-border/60 flex items-center gap-3 text-xs text-muted-foreground">
                  ...
                  {selectedCrops.map(c => {
                    ...
                    return (
                      <span key={key} className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-primary/15 text-primary text-xs font-medium">
                        {c.emoji} {c.cropName}
                        <button onClick={() => removeCrop(key)} className="ml-0.5 hover:text-red-500 transition-colors">
                          <X className="h-3 w-3" />
                        </button>
                      </span>
                    );
