...
router.get("/psgc/barangays", async (req, res) => {
  const cityCode = (req.query.city_code as string ?? "").trim();
  if (!cityCode) {
    res.status(400).json({ error: "city_code is required" });
    return;
  }
  try {
    let raw: any[];
    try {
      raw = await fetchPSGC(`/cities-municipalities/${cityCode}/barangays/`);
    } catch {
      try {
        raw = await fetchPSGC(`/cities/${cityCode}/barangays/`);
      } catch {
        raw = await fetchPSGC(`/municipalities/${cityCode}/barangays/`);
      }
    }
    const barangays = (raw || [])
      .map(b => ({
        code: b.code,
        name: b.name,
        cityCode: b.cityCode ?? b.municipalityCode ?? cityCode,
      }))
      .sort((a, b) => a.name.localeCompare(b.name));
    res.json(barangays);
  } catch (err) {
    req.log.error({ err }, "PSGC barangays fetch error");
    res.status(500).json({ error: "Failed to fetch barangays from PSGC" });
  }
});
